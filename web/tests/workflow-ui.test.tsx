...
test("workflow toolbar combines navigation, folder, run and panel controls", () => {
 ...
 const sidebar=html.slice(html.indexOf('data-slot="sidebar-trigger"'), html.indexOf('aria-label="일괄 실행"'))
 expect(sidebar).toContain('border-border')
 const panels=html.slice(html.indexOf('aria-label="패널 표시"'))
 expect((panels.match(/border-border/g) || []).length).toBeGreaterThanOrEqual(2)
 ...
 expect(html.indexOf('폴더 열기')).toBeGreaterThan(html.indexOf('toolbar-status-content'))
