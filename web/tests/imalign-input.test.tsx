...
test("alignment uses reference minus input, preserves row order and rejects invalid numbers", () => {
  ...
  expect(parsePairs("NaN 1")).toBeNull()
  expect(parsePairs("1 2 3")).toBeNull()
})
test("inline alignment inputs and bindings survive workflow, saved drafts and history", () => {
  ...
  const map = { ...emptyMap(), tasks: [t] },
    p = payloadFor(map, "a", cat)
  ...
  expect(
    workflowRequest(map, cat, "/data").nodes[0].payload.textInputs
  ).toEqual(p.textInputs)
  expect(
    restoreRun(emptyMap(), { id: "run", manifest: p }, cat, prefs).tasks[0]
      .draft.textInputs
  ).toEqual(p.textInputs)
})
...
test("shift rows follow input order even when reference is elsewhere or separate", () => {
  ...
  expect(
    pickAlignmentStar({ ...start, index: 1 }, [46, 19], "ref", ids).value
  ).toBe("? ?\n0 0\n? ?\n")
  const separate = pickAlignmentStar(
    { index: -1, value: "" },
    [45, 18],
    "ref",
    ["b"]
  ...
  expect(pickAlignmentStar(separate, [53, 13], "ref", ["b"]).value).toBe(
    "-8 5\n"
  )
  expect(
    pickAlignmentStar({ index: -1, value: "" }, [45, 18], "ref", ["ref"]).value
  ).toBe("0 0\n")
})

test("simultaneous star picks preserve image order and recompute when the reference changes", () => {
  const picks = {
    0: [53, 13] as [number, number],
    2: [42, 20] as [number, number],
  }
  expect(shiftsFromStars([45, 18], picks, "ref", ["b", "ref", "c"])).toBe(
    "-8 5\n0 0\n3 -2\n"
  )
  expect(shiftsFromStars([46, 19], picks, "ref", ["b", "ref", "c"])).toBe(
    "-7 6\n0 0\n4 -1\n"
  )
  expect(
    shiftsFromStars([45, 18], { 2: [42, 20] }, "ref", ["b", "ref", "c"])
  ).toBe("? ?\n0 0\n3 -2\n")
})

test("coordinate rows retain invalid rows and original numbering for deletion and replacement", () => {
  expect(alignmentRows("12 18\n? ?\n\n24.5 30\n")).toEqual([
    { text: "12 18", point: [12, 18] },
    { text: "? ?", point: null },
    { text: "24.5 30", point: [24.5, 30] },
  ])
  expect(alignmentRows("0 2\nNaN 3\n4 5 6").map((r) => r.point)).toEqual([
    null,
    null,
    null,
  ])
